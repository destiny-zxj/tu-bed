...
- **数据库**：MySQL
- **存储**：本地磁盘（默认）或七牛云对象存储（Qiniu，按日期分目录），通过 `DRIVE` 配置切换
...
- 图片存储于本地磁盘（默认）或七牛云对象存储，按 `app_tubed/YYYYMM/DD/` 分目录
- 一键 Docker 部署
...
│       │   └── storage.py   # 存储驱动：本地磁盘 / 七牛云对象存储上传与删除
...
> 注意：默认使用本地存储（`DRIVE=local`），图片保存在 `UPLOAD_DIR` 目录并通过 `/uploads` 路径访问。如需切换为七牛云，将 `DRIVE` 设为 `qiniu`，并在 `docker-compose.yml` 的 `backend` 服务中配置 `QINIU_ACCESS_KEY`、`QINIU_SECRET_KEY`、`QINIU_BUCKET`、`QINIU_DOMAIN`，否则上传接口会报"七牛云存储未正确配置"。
...
返回结果中的 `url` 即为图片公开访问地址（本地存储为 `/uploads/...`，七牛云为外链）。
...
| `DRIVE` | 存储驱动：`local`（本地磁盘，默认）或 `qiniu`（七牛云对象存储） |
